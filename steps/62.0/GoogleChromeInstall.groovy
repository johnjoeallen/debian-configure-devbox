...
final String stepKey = "googleChrome"
...
def ensureKey = {
  ...
  sh("sudo install -d -m 0755 -o root -g root /etc/apt/trusted.gpg.d")
  def needKey = sh("test -f '${keyPath}'").code != 0
  if (needKey) {
    def fetchKey = sh("curl -fsSL https://dl.google.com/linux/linux_signing_key.pub | sudo gpg --dearmor -o '${keyPath}'")
    if (fetchKey.code != 0) {
...
def ensureRepo = {
  ...
  sh("sudo install -d -m 0755 -o root -g root /etc/apt/sources.list.d")
  if (!current.exists() || current.text != desired) {
    def writeCmd = """cat <<'EOF' | sudo tee ${listPath} >/dev/null
${desired.trim()}
EOF"""
...
if (sh("sudo apt-get update -y").code != 0) {
  System.err.println("apt-get update failed for Google Chrome")
  System.exit(1)
}

if (sh("DEBIAN_FRONTEND=noninteractive sudo apt-get install -y google-chrome-stable").code != 0) {

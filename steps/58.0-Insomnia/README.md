...
3. Download the release `.deb` from GitHub into a dedicated directory under `/tmp` for the invoking user and install it via `apt`.
4. Remove the temporary directory on success (it remains on failure for debugging) and exit with code 10 after installation.
